...
point = [(37.6,126.9),(37.6,127.0),(37.5,126.9),(37.5,127.0),(37.5,127.1),(37.4,126.9),(37.4,127.0),(37.4,127.1)]
...
def targetmodel(lat,lon,train_df, val_df, test_df, isTrained) :
    ...
    if isTrained:
        model = keras.models.load_model(str(lat)+'_'+str(lon)+'_'+'model')
    else:
        ...
        with strategy.scope():
            ...
            # Fit the model
            model_history = model.fit(train_x, train_y, epochs=100, batch_size=5, validation_data=(val_x, val_y),
                                      verbose=1)
            plot_history([(str(lat)+str(lon), model_history)], str(lat)+str(lon))

            # Save model
            model.save(str(lat)+'_'+str(lon) +'_'+ 'model')

            # Calculate predictions
            ldmodel = keras.models.load_model(str(lat)+'_'+str(lon) +'_'+'model')
            PredTrainSet = pd.DataFrame(ldmodel.predict(train_x))
            PredValSet = pd.DataFrame(ldmodel.predict(val_x))
            idx = 0
...
def allchemmodel(models, test_x, test_y,t) :
    ...
    for i in range(0,len(test_x)-t):
        ...
        for step in range(0, t - 1):
            ...
            pred_next = models.predict(input)
        ...
        input = np.array(split_test[t - 1:t])
        print(input)
        pred_next = models.predict(input)
...
def run(lat, lon, t) :
    ...
    model, test_x, test_y = targetmodel(lat, lon, train_df, val_df, test_df, True)
    df_PM25, df_PM10, df_SO2, df_NO2, df_CO, df_O3 = allchemmodel(model, test_x, test_y,t)
    tsplot.actual_plot(df_PM25,len(test_df), lat, lon,'PM25',train_mean.CPM25,train_std.CPM25,'ug/m^3',t)
    tsplot.actual_plot(df_PM10, len(test_df), lat, lon, 'PM10',train_mean.CPM10,train_std.CPM10,'ug/m^3',t)
    tsplot.actual_plot(df_SO2, len(test_df), lat, lon, 'SO2',train_mean.CSO2,train_std.CSO2,'ug/m^3',t)
    tsplot.actual_plot(df_NO2, len(test_df), lat, lon, 'NO2',train_mean.CNO2,train_std.CNO2,'ug/m^3',t)
    tsplot.actual_plot(df_CO, len(test_df), lat, lon, 'CO',train_mean.CCO,train_std.CCO,'ug/m^3',t)
    tsplot.actual_plot(df_O3, len(test_df), lat, lon, 'O3',train_mean.CO3,train_std.CO3,'ug/m^3',t)
